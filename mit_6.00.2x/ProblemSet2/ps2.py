...
class RectangularRoom(object):
    ...
    Decide what fields you will use and decide how the following operations
    are to be performed:

    -   Initializing the object
    -   Marking an appropriate tile as cleaned when a robot moves to a given
            position (casting floats to ints - and/or the function
    ...
    def isTileCleaned(self, m, n):
        ...
        m = float("%02f" % m)
        n = float("%02f" % n)
        for t in self.clean_tiles:
            if t.getX() == m and t.getY() == n:
                return True
        return False
